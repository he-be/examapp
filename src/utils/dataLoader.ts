...
async function fetchJSON<T>(url: string): Promise<T> {
  try {
    ...
    throw new NetworkErrorImpl(`Failed to fetch or parse JSON from ${url}`, undefined, {
      originalError: error,
    });
  }
}
...
function validateMMLUQuestion(question: any): question is MMLUQuestion {
  ...
  const mmluQuestion = question as MMLUQuestion;
  if (!Array.isArray(mmluQuestion.choices) || mmluQuestion.choices.length === 0) return false;
  if (typeof mmluQuestion.correctAnswer !== 'number') return false;
  if (mmluQuestion.correctAnswer < 0 || mmluQuestion.correctAnswer >= mmluQuestion.choices.length)
    return false;
  if (!mmluQuestion.category || typeof mmluQuestion.category !== 'string') return false;
...
function validateGSM8KQuestion(question: any): question is GSM8KQuestion {
  ...
  const gsm8kQuestion = question as GSM8KQuestion;
  if (typeof gsm8kQuestion.correctAnswer !== 'number') return false;
  if (!Array.isArray(gsm8kQuestion.chainOfThought)) return false;
...
export async function preloadAllData(): Promise<void> {
  const loadPromises = [
    ...
    loadBigBenchQuestions().catch((error) =>
      console.warn('Failed to preload BIG-Bench-Hard:', error)
    ),
